    ...
    function match_users($mode='findmatches') {
        ...
        if ($error == '') {
            ...
            if ($listmode == 'findmatches') {
                ...
                $query = 'SELECT DISTINCT * FROM `users` WHERE '. $obada .' AND '.$gender_query.' `active` = "1" AND `id` NOT IN (SELECT `block_userid` FROM `blocks` WHERE `user_id` = ' . self::ActiveUser()->id . ') '. $execludes .' AND `id` NOT IN (SELECT `like_userid` FROM `likes` WHERE `user_id` = ' . self::ActiveUser()->id . ' AND `id` <> "' . self::ActiveUser()->id . '" ) ORDER BY `id` DESC LIMIT ' . $limit ;
            }
            //echo $query;
