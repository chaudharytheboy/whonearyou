    ...
    function register() {
        ...
        if ($users) {
            if (isset($_POST) && !empty($_POST)) {
                ...
                if ($error == '') {
                    $regestered_user = $users->register($_POST);
...
//    }
    ...
    function like() {
        ...
        $is_user_matches = $db->where('user_id', $userid)->where('like_userid', self::ActiveUser()->id)->getOne('likes', array('id'));
        $id              = $db->where('user_id', self::ActiveUser()->id)->where('like_userid', $userid)->getValue('likes', 'id');
        $saved           = false;
        if ($id > 0) {
        ...
        if ($saved) {
            if ($id == 0) {
                ...
                if ($is_user_matches > 0) {
                    $Notification->createNotification($web_device_id, self::ActiveUser()->id, $userid, 'got_new_match', '', '/@' . self::ActiveUser()->username);
                    $Notification->createNotification($web_device_id, $userid, self::ActiveUser()->id, 'got_new_match', '', '/@' . $username);
                }
                if(self::Config()->pro_system === "1") {
                    if (self::ActiveUser()->is_pro === "1") {
                        $db->where('notifier_id', self::ActiveUser()->id)->where('recipient_id', $userid)->where('type', 'like')->delete('notifications');
                        $Notification->createNotification($web_device_id, self::ActiveUser()->id, $userid, 'like', '', '/@' . self::ActiveUser()->username);
                    }
                }else{
                    $db->where('notifier_id', self::ActiveUser()->id)->where('recipient_id', $userid)->where('type', 'like')->delete('notifications');
                    $Notification->createNotification($web_device_id, self::ActiveUser()->id, $userid, 'like', '', '/@' . self::ActiveUser()->username);
    ...
    function remove_like() {
        ...
        $deleted = $db->where('user_id', self::ActiveUser()->id)->where('like_userid', $userid)->where('is_like', '1')->delete('likes');
        if ($deleted) {
            $db->where('notifier_id', $userid)->where('recipient_id', self::ActiveUser()->id)->where('type', 'like')->delete('notifications');
            $db->where('notifier_id', $userid)->where('recipient_id', self::ActiveUser()->id)->where('type', 'got_new_match')->delete('notifications');
            return array(
                'status' => 200,
                'like_text' => __('like'),
                'userid' => $userid
    ...
    function dislike() {
        ...
        if ($id > 0) {
            ...
            if(isUserDisliked($userid) === false) {
                $saved = $db->insert('likes', array(
                    'user_id' => self::ActiveUser()->id,
                    'like_userid' => $userid,
                    'is_like' => 0,
    ...
    function remove_dislike() {
        ...
        $deleted = $db->where('user_id', self::ActiveUser()->id)->where('like_userid', $userid)->where('is_dislike', '1')->delete('likes');
        if ($deleted) {
            $db->where('notifier_id', self::ActiveUser()->id)->where('recipient_id', $userid)->where('type', 'dislike')->delete('notifications');
            return array(
                'status' => 200,
                'dislike_text' => __('Dislike'),
                'userid' => $userid
