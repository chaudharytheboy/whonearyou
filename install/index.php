...
function check_($check) {
    ...
    return $check             = array('status' => 'SUCCESS', 'url' => $siteurl, 'code' => $check);;
}
...
$config_file = file_put_contents($config_file_name, $file_content);
    if ($config_file) {
        ...
        if ($query) {
            ...
            $query_one  = mysqli_query($con, "UPDATE `options` SET `option_value` = '" . mysqli_real_escape_string($con, $_POST['siteName']). "' WHERE `option_name` = 'site_name'");
            $query_one .= mysqli_query($con, "UPDATE `options` SET `option_value` = '" . mysqli_real_escape_string($con, $_POST['siteTitle']). "' WHERE `option_name` = 'default_title'");
            $query_one .= mysqli_query($con, "UPDATE `options` SET `option_value` = '" . mysqli_real_escape_string($con, $_POST['siteEmail']). "' WHERE `option_name` = 'siteEmail'");
            $query_one .= mysqli_query($con, "INSERT INTO `users` (`username`,`password`, `email`, `admin`, `active`, `verified`, `registered`, `start_up`, `created_at`,`src`,`type`,`privacy_show_profile_on_google`,`privacy_show_profile_random_users`,`privacy_show_profile_match_profiles`,`smscode`,`email_code`,`gender`) VALUES ('" . mysqli_real_escape_string($con, $_POST['admin_username']). "', '" . mysqli_real_escape_string($con, password_hash($_POST['admin_password'], PASSWORD_DEFAULT, array('cost' => 11)) ) . "','" . mysqli_real_escape_string($con, $_POST['siteEmail']) . "','1', '1', '1', '".time()."', '3','".date('Y-m-d H:i:s')."','installation','admin','1','1','1','".rand(1111, 9999)."','".rand(1111, 9999)."','4525');");
