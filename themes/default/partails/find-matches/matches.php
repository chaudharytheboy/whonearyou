    ...
    <div class="row">
        <div class="col <?php if( $mode == 'hot' ){?>s12<?php }else{ ?>s12 m7<?php }?>">
            <div class="mtc_usrd_slider">
				...
				<?php if( $mode == 'hot' ){?>
					<?php $x = 0; foreach ($matche->mediafiles as $key => $mfile){ if( $x > 1 ) { continue; } else {?>
						<img alt="<?php echo $matche->username;?>" src="<?php echo $mfile['avater'];?>" <?php if(count($matche->mediafiles) == 1){ echo 'style="width: 100%;"';}?>>
					<?php }$x++;} ?>
				<?php }else{ ?>
